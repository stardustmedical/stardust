...
def get_image(image_url):
    ...
    if response.status_code == 200 and 'image' in response.headers['Content-Type']:
        ...
        print(f"Error: Failed to retrieve image from URL {image_url} with status code {response.status_code}")
        return None
...
image_embeddings = [embedding for sublist in image_data_df.img_embeddings.tolist() for embedding in sublist]
